...
fn rs_georeference_doc() -> Documentation {
    Documentation::builder(
        DOC_SECTION_OTHER,
        "Returns the georeference metadata of raster as a string in GDAL or ESRI format. Default is GDAL if not specified.".to_string(),
        "RS_GeoReference(raster: Raster, format: String = 'GDAL')".to_string(),
    )
    .with_argument("raster", "Raster: Input raster")
    .with_argument("format", "String: Output format, either 'GDAL' (default) or 'ESRI'")
    .with_sql_example("SELECT RS_GeoReference(RS_Example())".to_string())
    .build()
}

/// Format type for GeoReference output
...
impl SedonaScalarKernel for RsGeoReferenceOneArg {
    ...
    fn invoke_batch(
        ...
        let preallocate_bytes = 100 * executor.num_iterations();
        let mut builder =
...
mod tests {
    ...
    fn udf_georeference_gdal_default() {
        ...
        // Check first raster (index 0):
        // scale_x=0.0, scale_y=-0.0, skew_x=0.0, skew_y=0.0, upper_left_x=1.0, upper_left_y=2.0
        let expected_0 = "0.000000\n0.000000\n0.000000\n-0.000000\n1.000000\n2.000000";
        assert_eq!(string_array.value(0), expected_0);

        // Second raster is null
        assert!(string_array.is_null(1));

        // Check third raster (index 2):
        // scale_x=0.2, scale_y=-0.4, skew_x=0.06, skew_y=0.08, upper_left_x=3.0, upper_left_y=4.0
        let expected_2 = "0.200000\n0.080000\n0.060000\n-0.400000\n3.000000\n4.000000";
        assert_eq!(string_array.value(2), expected_2);
    }
    ...
    fn udf_georeference_esri() {
        ...
        // Check third raster (index 2) with ESRI format:
        // scale_x=0.2, scale_y=-0.4, skew_x=0.06, skew_y=0.08, upper_left_x=3.0, upper_left_y=4.0
        // ESRI: upper_left_x + scale_x * 0.5 = 3.0 + 0.1 = 3.1
        // ESRI: upper_left_y + scale_y * 0.5 = 4.0 + (-0.2) = 3.8
        let expected_2 = "0.200000\n0.080000\n0.060000\n-0.400000\n3.100000\n3.800000";
        assert_eq!(string_array.value(2), expected_2);
    }
