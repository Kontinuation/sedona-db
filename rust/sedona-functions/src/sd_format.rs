    ...
    match columnar_value {
        ColumnarValue::Array(array) => {
            ...
            for i in 0..raster_array.len() {
                ...
                let has_outdb = bands
                    .iter()
                    .any(|band| matches!(band.metadata().storage_type(), StorageType::OutDbRef));
...
mod tests {
    ...
    fn sd_format_formats_raster_columns() {
        ...
        assert_eq!(formatted.value(0),
            "Raster[w=1, h=2, ul=(1.000000, 2.000000), scale=(0.000000, -0.000000), skew=(0.000000, 0.000000), bands=1, outdb=false]"
        );
