...
use sedona_spatial_join::partitioning::{rsgrove::RSGrovePartitioner, SpatialPartitioner};
...
fn bench_partition_queries(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("rsgrove_partition_queries");
...
fn build_partitioners(
    samples: &[BoundingBox],
    extent: &BoundingBox,
) -> Vec<(usize, RSGrovePartitioner)> {
    DEPTHS
        .iter()
        .map(|&depth| {
            let partitioner = RSGrovePartitioner::build(
                samples.iter().cloned(),
                MAX_ITEMS_PER_NODE,
                depth,
                extent.clone(),
            )
            .expect("failed to build RSGrove partitioner for benchmark");
            (depth, partitioner)
        })
        .collect()
}
...
criterion_group!(rsgrove_partition, bench_partition_queries);
criterion_main!(rsgrove_partition);
