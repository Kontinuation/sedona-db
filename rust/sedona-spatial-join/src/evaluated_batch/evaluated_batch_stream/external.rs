            ...
            match &mut self_mut.state {
                State::UnInitialized(_) => {
                    let State::UnInitialized(spill_file) =
                        std::mem::replace(&mut self_mut.state, State::Finished)
                    else {
                        unreachable!()
                    };
                    let task =
...
mod tests {
    ...
    async fn test_external_stream_creation() -> Result<()> {
        ...
        let stream = ExternalEvaluatedBatchStream::try_new(spill_file)?;
    ...
    async fn test_external_stream_single_batch() -> Result<()> {
        let spill_file = create_spill_file_with_batches(1).await?;
        let mut stream = ExternalEvaluatedBatchStream::try_new(spill_file)?;

        // Read the first batch
        let batch1 = stream.next().await;
        assert!(batch1.is_some());
        let batch1 = batch1.unwrap()?;
        assert_eq!(batch1.num_rows(), 3);

        // Verify the first row ID
        let id_array = batch1
            .batch
            .column(0)
            .as_any()
    ...
    async fn test_external_stream_concurrent_access() -> Result<()> {
        ...
        let mut stream1 = ExternalEvaluatedBatchStream::try_new(file1)?;
        let mut stream2 = ExternalEvaluatedBatchStream::try_new(file2)?;
