    ...
    fn invoke_batch_from_args(
        ...
        with_gdal(|gdal| {
            ...
            executor.execute_raster_wkb_crs_void(|raster_opt, wkb_opt, maybe_point_crs| {
                ...
                let maybe_point_crs = resolve_crs(maybe_point_crs)?;
                let raster_crs = resolve_crs(raster_crs)?;

                let point_wkb =
                    crs_transform_wkb(point_wkb, maybe_point_crs.as_ref(), raster_crs.as_ref())?;
...
mod tests {
    ...
    fn test_rs_value_point_crs_transform() {
        ...
        let (x_merc, y_merc) = crs_transform_coord((lon, lat), "OGC:CRS84", "EPSG:3857").unwrap();
