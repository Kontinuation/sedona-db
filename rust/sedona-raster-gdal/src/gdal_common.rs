...
pub fn gdal_to_band_data_type(gdal_type: GdalDataType) -> Result<BandDataType> {
    match gdal_type {
        ...
        _ =>
        // CInt16, CInt32, CFloat32, CFloat64, Unknown are unsupported
        {
...
///
/// # Returns
/// A [`gdal::Dataset`] that provides access to the GDAL dataset.
///
/// # Errors
/// Returns an error if:
...
pub fn create_outdb_source(url: &str, open_options: Option<&[&str]>) -> Result<gdal::Dataset> {
