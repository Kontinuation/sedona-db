...
impl<'a> RasterBandReader<'a> {
        ...
        match storage_type {
            ...
            StorageType::OutDbRef => {
                ...
                let buffer = gdal_band
                    ...
                    .map_err(|e| {
                        DataFusionError::Execution(format!("Failed to read pixel: {e}"))
                    })?;
                Ok(buffer.data()[0])
            }
        }
        ...
        match storage_type {
            ...
            StorageType::OutDbRef => {
                ...
                let buffer = gdal_band
                    ...
                    .map_err(|e| {
                        DataFusionError::Execution(format!("Failed to read window: {e}"))
                    })?;
    ...
    fn ensure_dataset(&mut self) -> Result<&RasterDataset<'a>> {
        if self.dataset.is_none() {
            ...
            let dataset = provider.raster_ref_to_gdal(self.raster).map_err(|e| {
    ...
    fn raster_has_outdb(&self) -> Result<bool> {
        ...
        for idx in 1..=bands.len() {
            let band = bands.band(idx).map_err(|e| {
                DataFusionError::Execution(format!("Failed to get band {}: {e}", idx))
            })?;
...
fn read_gdal_bytes<T: gdal::raster::GdalType + ToLeBytes + Copy>(
    band: &gdal::raster::RasterBand,
    pixel_count: usize,
) -> Result<Vec<u8>> {
    ...
    let buffer = band
        ...
        .map_err(|e| DataFusionError::Execution(format!("Failed to read band: {e}")))?;
...
#[allow(dead_code)]
fn read_gdal_window_bytes<T: gdal::raster::GdalType + ToLeBytes + Copy>(
    band: &gdal::raster::RasterBand,
    offset: (usize, usize),
    size: (usize, usize),
) -> Result<Vec<u8>> {
    let buffer = band
        ...
        .map_err(|e| DataFusionError::Execution(format!("Failed to read window: {e}")))?;
    let values = buffer.data();
